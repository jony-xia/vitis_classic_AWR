...
void Host_asyncEventHandler(uint16_t msgId, uint16_t sbId,uint16_t sbLen, uint16_t* payload);int32_t Host_osiSleep(uint32_t delayInMs);
int Host_computeCRC(unsigned char* data, unsigned int dataLen, unsigned char crcLen,unsigned char* outCrc);
int32_t Host_registerInterruptHandler(uint8_t deviceIndex,RL_P_EVENT_HANDLER pHandler,void* pValue);
...
rlInt32_t Host_debugPrint(const rlInt8_t* format, ...);
#endif // HOST_H
